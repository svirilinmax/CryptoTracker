...
class Asset(Base):
    ...
    symbol = Column(String, index=True)                 # Название валюты (например: "bitcoin", "ethereum")
...
class PriceHistory(Base):
    ...
    asset_id = Column(Integer, ForeignKey("assets.id")) # Ссылка на актив (внешний ключ)
    price = Column(Float)                               # Цена актива в момент записи
    recorded_at = Column(DateTime, default=datetime.utcnow)  # Временная записи

    asset = relationship("Asset", back_populates="price_history")  # Позволяет получить доступ к данным актива из записи цены
